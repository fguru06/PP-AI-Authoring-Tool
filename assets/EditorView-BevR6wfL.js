...
Make all content specific to "${r}" — no generic placeholders.`,v});nt(ie,r=>{Ee.value||(ae.value=r)},{immediate:!0});function ge(){ae.value=ie.value,Ee.value=!1}const we=j(""),pe=j("intermediate"),be=j("multiple-choice"),re=j(""),N=j([]),S=j(""),R=j(!1),ke=M(()=>{const r=we.value.trim()||"[your topic]",n={beginner:"Beginner",intermediate:"Intermediate",advanced:"Advanced"},v={"multiple-choice":"Multiple Choice","true-false":"True / False",mixed:"Mixed (MCQ + T/F)"};let I=`Generate ${Ie.value} ${n[pe.value]} ${v[be.value]} quiz questions about "${r}".
...
Create exactly 3 distinct creative slide directions for a "${V.value}" slide about "${r}" using a ${D.value} layout.
...
${O}
${H!=="pdf"?'<script src="script.js"><\/script>':`<script>${D}<\/script>`}
</body>
