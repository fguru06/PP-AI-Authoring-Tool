...
`):[]).map(v=>p(v)).filter(Boolean)}function h(r,o,v){const E=[...r];for(;E.length<o;)E.push(v(E.length));return E.slice(0,o)}function g(r,o){const v=Array.isArray(r)?r.map((b,ae)=>({title:p(b==null?void 0:b.title,`Card ${ae+1}`),body:p((b==null?void 0:b.body)||(b==null?void 0:b.text)||(b==null?void 0:b.description),"Add supporting detail.")})).filter(b=>b.title||b.body):[],E=o.map((b,ae)=>({title:`Point ${ae+1}`,body:b}));return h(v.length?v:E,3,b=>({title:`Point ${b+1}`,body:"Add supporting detail."}))}function j(r,o){const v=y(r==null?void 0:r.leftPoints),E=y(r==null?void 0:r.rightPoints),b=o.slice(0,Math.max(1,Math.ceil(o.length/2))),ae=o.slice(Math.max(1,Math.ceil(o.length/2)));return{leftTitle:p(r==null?void 0:r.leftTitle,"Option A"),leftPoints:h(v.length?v:b,3,Te=>`Left point ${Te+1}`),rightTitle:p(r==null?void 0:r.rightTitle,"Option B"),rightPoints:h(E.length?E:ae,3,Te=>`Right point ${Te+1}`)}}function ne(r,o){const v=Array.isArray(r)?r.map((b,ae)=>({value:p(b==null?void 0:b.value,`${ae+1}`),label:p(b==null?void 0:b.label,`Metric ${ae+1}`)})).filter(b=>b.value||b.label):[],E=o.map((b,ae)=>{const[Te,...et]=b.split(/[:\-]/);return{value:p(Te,`${ae+1}`),label:p(et.join(" ").trim(),b)}});return h(v.length?v:E,3,b=>({value:`${b+1}`,label:`Metric ${b+1}`}))}function le(r,o){const v=Array.isArray(r)?r.map((b,ae)=>({title:p(b==null?void 0:b.title,`Phase ${ae+1}`),detail:p((b==null?void 0:b.detail)||(b==null?void 0:b.description),"Explain the milestone.")})).filter(b=>b.title||b.detail):[],E=o.map((b,ae)=>({title:`Phase ${ae+1}`,detail:b}));return h(v.length?v:E,4,b=>({title:`Phase ${b+1}`,detail:"Explain the milestone."}))}function W(r,o){const v=Array.isArray(r)?r.map((b,ae)=>({question:p(b==null?void 0:b.question,`Question ${ae+1}?`),answer:p(b==null?void 0:b.answer,"Add the short answer here.")})).filter(b=>b.question||b.answer):[],E=o.map((b,ae)=>({question:`Question ${ae+1}?`,answer:b}));return h(v.length?v:E,3,b=>({question:`Question ${b+1}?`,answer:"Add the short answer here."}))}function me(r,o){const v=Array.isArray(r)?r.map((b,ae)=>({title:p(b==null?void 0:b.title,`Step ${ae+1}`),detail:p((b==null?void 0:b.detail)||(b==null?void 0:b.description),"Add the step detail here.")})).filter(b=>b.title||b.detail):[],E=o.map((b,ae)=>({title:`Step ${ae+1}`,detail:b}));return h(v.length?v:E,3,b=>({title:`Step ${b+1}`,detail:"Add the step detail here."}))}const z=Gn(),N=lt(),a=it(),M=P(()=>z.apiProvider==="gemini"?"Gemini":"OpenAI"),_=P(()=>z.apiProvider==="gemini"?"AIza...":"sk-..."),H=P(()=>z.apiProvider==="gemini"?"Gemini API keys from Google AI Studio work for text features on the free tier. We provide free image generation via Pollinations.ai when Gemini is selected. The key is stored locally in your browser and never sent to our servers.":"Key is stored locally in your browser. It is never sent to our servers."),f=B("generate"),U=B(""),oe=B(""),X=B("");Qe(f,()=>{X.value=""}),Qe(()=>N.aiPanelMode,r=>{r&&(f.value=r)},{immediate:!0});const we=B(4),R=B("general"),V=B("Spanish"),ce=B(""),ge=B(""),te=B(""),ke=B(!1),C=B("single"),L=B("classic"),D=B("mixed"),Y=B(5),ye=B("slide"),Be=B([]),Ie=B(""),_e=P(()=>s.find(r=>r.id===L.value)||s[0]),ee=P(()=>{const r=oe.value.trim()||"[your topic]";if(C.value==="deck"){let E=`Create a complete ${Y.value}-slide learning deck about "${r}".`;return ge.value.trim()&&(E+=`
